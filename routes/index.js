...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      if (file) {
        const extractedResumeText = await fileService.extractText(file);
      ...
      const apiResponse = await openaiService.getCareerSuggestions(
        combinedText
      );
...
router.get("/documentation", (req, res) => {
  res.render("documentation", {
    title: "CareerCraft Documentation",
  });
});
...
router.get("/suggestions/:id", async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching the suggestion." });
  }
});
...
router.delete("/suggestions/:id", async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the suggestion." });
  }
});
